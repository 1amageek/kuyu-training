...
| Generic rollout health preserves failure-reason histograms for downstream gates without interpreting profile-specific reason semantics. | `RolloutHealth.failureReasonCounts`, `RolloutHealthTests.rolloutHealthAggregatesFailureReasonCounts`, `../scripts/validate-kuyu-boundaries.sh`. |
