...
struct ManagedTrainingRunExecutorTests {
    ...
    @Test(.timeLimit(.minutes(1))) func resumeValidationFailureDoesNotLaunchOperation() async {
        ...
        do {
            _ = try await executor.resume(Self.makeResumeRequest(runID: TrainingRunID("executor-resume-rejected")))
    ...
    private static func makeContinuationSelection(_ artifactRoot: URL) -> TrainingContinuationSelection {
